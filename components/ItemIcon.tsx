...
function cleanDescription(desc: string) {
  return (desc || '')
    ...
    .replace(
      /<c=?(.*?)>(.+?)<\/c>/,
      (_, attr, content) => `<span style="${descStyles[attr.toLowerCase()] || ''}">${content}</span>`
    )
}
...
function useDupLocations(isDup: boolean, inventories: any, id?: number) {
  return useMemo(() => {
    ...
    return (inventories || []).filter(
      ({ inventory }) => inventory && inventory.some((i) => i && i.inventory.some(({ id: invId }) => invId === id))
    )
    ...
    <Tippy
      ...
      content={
        ...
        >
          ...
          />
          {isDup && dupLocations.length > 0 ? (
            <div style={{ marginTop: '0.5em' }}>
              Duplicated in:
              <ul style={{ margin: 3 }}>
                {dupLocations.map(({ character }) => (
                  <li key={character.name} style={{ margin: 0 }}>
          ...
          {(b.flags || []).includes('AccountBound') ? <div style={{ marginTop: '0.5em' }}>Account Bound</div> : null}
          {b.charges ? <div style={{ marginTop: '0.5em' }}>{b.charges} charges</div> : null}
          {b.bound_to ? <div style={{ marginTop: '0.5em' }}>Bound to: {b.bound_to}</div> : null}
          {b.id ? <div style={{ marginTop: '0.5em', textAlign: 'right', opacity: 0.5 }}>ID: {b.id}</div> : null}
        </div>
      }
      disabled={!b.name}
    >
