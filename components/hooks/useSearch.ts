...
export default function useSearch (bank: any, inventories: any, guildStashes: GuildItem[], search: string) {
  return useMemo(() => {
    ...
    const filteredGuildStashes = guildStashes.map((g) => {
      return Object.assign({}, g, {stash: g.stash.map((s) => {
        if (!s || !s.inventory) return s;
        return Object.assign({}, s, {inventory: s.inventory.filter((item) => item && item.name && item.name.toLowerCase().includes(search.toLowerCase()))})
      })})
    });
