...
async function getGuildsStashes(api: any, guildIds: string[]): Promise<GuildStash[]> {
  return Promise.all(
    guildIds.map(async (id) => {
      try {
        ...
        return typeof result === 'string' ? { stash: [], error: result, id } : { stash: result, error: undefined, id }
      } catch (e) {
        console.error(e)
...
async function getGuildsInfo(api: any, guildIds: string[]): Promise<GuildInfo[]> {
  return Promise.all(
    guildIds.map(async (id) =>
      api
        .guild(id)
  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        ...
        const [guildInfo, guilds]: [GuildInfo[], GuildStash[]] = await Promise.all([
        ...
        const merged = guilds
          .map((g) =>
            Object.assign<GuildStash, GuildInfo>(
              g,
              guildInfo.find((gi) => gi.id === g.id)
            )
          )
          .filter((g) => !g.error)
        const allItemIds = new Set<number>(
          merged
            .reduce(
              (acc, m) =>
                acc.concat(m.stash.reduce((stacc, s) => stacc.concat(s.inventory.map((item) => item && item.id)), [])),
              []
            )
            .filter(Boolean)
        )
